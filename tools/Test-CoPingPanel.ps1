...
foreach ($relPath in $expectedFiles) {
    ...
    $detail = if ($exists) {
        ...
        "Missing: expected at $fullPath"
    }

    $checks.Add(
        (New-CoPingCheck -Name "Path: $relPath" -Ok $exists -Detail $detail)
    )
...
if (Test-Path $edgeControlPath) {
    try {
        ...
        if (-not $json.mode) {
            ...
            $checks.Add(
                (New-CoPingCheck -Name 'EdgeControl.json: mode' -Ok $true -Detail ("mode = {0}" -f $json.mode))
            )
        }
    } catch {
        $hadError = $true
